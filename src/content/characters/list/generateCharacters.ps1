...
<# foreach ($character in $characters.GetEnumerator()) {
    if ($pairCharacters.ContainsKey($character.Key)) {
        foreach ($pairCharacter in $pairCharacters[$character.Key]) {
            $characterName = $pairCharacter.Value 
            $fileName = $pairCharacter.Key -replace "_", ""
            $id = $fileName.Substring(0, 1).ToLower() + $fileName.Substring(1)
            $imageName = $pairCharacter.Key

            if ($pairCharacter.Key -eq $character.Key) {
                $imagePath = $character.Key
            }
            else {
                $imagePath = $pairCharacter.Key
            }
...
const ${fileName}: ICharacter = {
  id: "$id",
  name: "$characterName",
  avatar: { ref: "/characters/${imagePath}/${imageName}_treeAvatar.jpg", alt: "$characterName" },
  mainImage: { ref: "/characters/${imagePath}/${imageName}_ref.jpg", alt: "$characterName" }
};
...
const ${fileName}: ICharacter = {
  id: "$id",
  name: "$characterName",
  avatar: { ref: "/characters/${imagePath}/${imagePath}_treeAvatar.jpg", alt: "$characterName" },
  mainImage: { ref: "/characters/${imagePath}/${imagePath}_ref.jpg", alt: "$characterName" }
};
